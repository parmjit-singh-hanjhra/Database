...
SELECT w_p.country_name AS country_name, w_p.party_id AS party_id, w_p.num_wins AS num_wins
FROM winning_parties w_p, avg_wins a_w 
WHERE (w_p.country_name = a_w.country_name) AND (w_p.num_wins > (3*a_w.avg_wins));
...
//SELECT l_e_w.country_name AS countryName, p.name AS partyName, p_f.family AS partyFamily, b_a_p.num_wins AS wonElections,
//	   l_e_w.election_id AS mostRecentlyWonElectionId, CAST(DATE_PART('YEAR', l_e_w.most_recent_date) AS INT)  
//FROM (((latest_election_won l_e_w JOIN best_avg_parties b_a_p ON  (l_e_w.party_id = b_a_p.party_id)) JOIN 
//party p ON (l_e_w.party_id = p.id)) LEFT JOIN party_family p_f ON (l_e_w.party_id = p_f.party_id));
